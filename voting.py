...
username_label.grid(row=2, column=0, padx=10, pady=5, sticky=tk.W)
username_entry = tk.Entry(root)
username_entry.grid(row=2, column=1, padx=10, pady=5)

# Pssword label and entry
...
role_label.grid(row=4, column=0, padx=10, pady=5, sticky=tk.W)
role_var = tk.StringVar()
role_var.set("user")
role_menu = tk.OptionMenu(root, role_var, "user", "admin")
role_menu.grid(row=4, column=1, padx=10, pady=5)
...
candidates_label = tk.Label(root, text="Select Candidate:")
candidates_label.grid(row=6, column=0, padx=10, pady=5, sticky=tk.W)
...
if candidate_names:  # Check if candidate names exist
    candidate_menu = tk.OptionMenu(root, candidate_var, *candidate_names)
    candidate_menu.grid(row=6, column=1, padx=10, pady=5)
else:
    candidate_menu = tk.OptionMenu(root, candidate_var, "No candidates")
    candidate_menu.grid(row=6, column=1, padx=10, pady=5)

# Add new candidate entry and button
new_candidate_label = tk.Label(root, text="Enter New Candidate:")
new_candidate_label.grid(row=7, column=0, padx=10, pady=5, sticky=tk.W)
new_candidate_entry = tk.Entry(root)
new_candidate_entry.grid(row=7, column=1, padx=10, pady=5)
add_candidate_button = tk.Button(root, text="Add Candidate", command=add_candidate, state=tk.DISABLED)
add_candidate_button.grid(row=8, column=0, columnspan=2, pady=5)

# Cast vote button (disabled initially)
cast_vote_button = tk.Button(root, text="Cast Vote", command=cast_vote, state=tk.DISABLED)
cast_vote_button.grid(row=9, column=0, columnspan=2, pady=5)

# Display results button
results_button = tk.Button(root, text="Display Results", command=display_results, state=tk.DISABLED)
results_button.grid(row=10, column=0, columnspan=2, pady=5)

# New Election / Reset Data button (disabled initially)
reset_data_button = tk.Button(root, text="New Election / Reset Data", command=reset_data, state=tk.DISABLED)
reset_data_button.grid(row=11, column=0, columnspan=2, pady=5)
